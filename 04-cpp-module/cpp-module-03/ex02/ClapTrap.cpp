...
void ClapTrap::takeDamage(unsigned int amount)
{
	...
	{
		std::cout << "[ " << C_NAME << " ] "
		<< "[ " << BYEL << std::setw(W_SIZE) << "Not Damaged" << EOC << " ] "
		<< _name << " ( Already Dead )" << std::endl;
		return ;
	}
	if (_hp <= amount)
...
void ClapTrap::beRepaired(unsigned int amount)
{
	...
	{
		std::cout << "[ " << C_NAME << " ] "
		<< "[ " << BYEL << std::setw(W_SIZE) << "Not Reapired" << EOC << " ] "
		<< _name << " ( Already Dead )" << std::endl;
		return ;
	}
	...
	std::cout << "[ " << C_NAME << " ] "
	<< "[ " << GRN << std::setw(W_SIZE) << "Repaired" << EOC << " ] "
	<< _name << " ( +" << std::setw(2) << amount << " --> "
	<< std::setw(2) << _hp << " )" << std::endl;
}

void ClapTrap::print_status(std::ostream& o) const
{
