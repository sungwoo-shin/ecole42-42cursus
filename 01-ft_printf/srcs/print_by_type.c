...
/*   Updated: 2021/09/26 21:12:03 by sshin            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static int		print_char(char c, t_info *info)
{
...
static int		print_str_by_prec(char *str, int len_to_print_str)
{
	int		i;
	int		ret;
	...
	{
		ret += write(STDOUT_FILENO, &str[i], 1);
		++i;
	}
	return (ret);
}

static int		print_str(char *str, t_info *info)
{
...
int		print_padding(t_info *info, int len_to_print_padding)
{
	...
	{
		ret += write(STDOUT_FILENO, &padding, 1);
