...
/*   Updated: 2021/09/26 21:11:38 by sshin            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static void	get_digit_info(const char fmt, t_info *info)
{
	if (info->m_prec == -1)
...
static void	get_wildcard_info(va_list ap, t_info *info)
{
	if (info->m_prec == -1)
	{
...
static void	get_opt_info(const char fmt, va_list ap, t_info *info)
{
	if (fmt == '0' && info->m_width == 0 && info->m_prec == -1)
		...
		get_wildcard_info(ap, info);
}

static void	parse_fmt(const char **fmt, va_list ap, t_info *info)
{
	const char *m_type;
