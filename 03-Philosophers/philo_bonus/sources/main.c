...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/09/22 15:49:09 by sshin             #+#    #+#             */
/*   Updated: 2021/09/30 12:36:42 by sshin            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	feed_philo(t_info *info)
{
	...
	while (i < info->nbr_philo)
	{
		...
		{
			if (pthread_create(&info->philo[info->i].th, NULL, monitor_death, info) || \
				pthread_detach(info->philo[info->i].th))
				sem_post(info->sem_dining);
			routine(info);
			exit(VALID);
		}
