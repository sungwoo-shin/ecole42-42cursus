...
/*   Updated: 2021/09/26 20:46:31 by sshin            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "get_next_line.h"

static int	get_idx_to_split(char *backup_fd)
{
	int idx;
...
static int	read_file(int fd, char **backup_fd, int *idx_to_split)
{
	char 	*buf;
	char 	*tmp;
	int		read_size;
...
static int	split_line(char **backup_fd, char **line, int idx_to_split)
{
	char *tmp;
...
static int	assign_last_line(char **backup_fd, char **line)
{
	if (*backup_fd)
	{
		*line = *backup_fd;
